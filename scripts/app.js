...
function makeSurePlaylistExists(name, callback) {
    ...
    if (name in importColl.playlists) {
        var set = {
            name: name,
            public: "public" in importColl.playlists[name] ? importColl.playlists[name].public : false, // for backwards compatibility
            description: "description" in importColl.playlists[name] ? importColl.playlists[name].description : null // for backwards compatibility
        }
    }
    else {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            updatePlaylistImage(response.name, response.id)
            callback(true);
        },
        fail: function () {
            callback(false);
...
function updatePlaylistImage(name, id) {
    if (!(name in importColl.playlists) || importColl.playlists[name].images == null) return

    fetch(importColl.playlists[name].images)
        .then(response => response.blob())
        .then(blob => {
            let reader = new FileReader();
            reader.onloadend = function () {
                let base64data = reader.result.split(',')[1];
                fetch('https://api.spotify.com/v1/users/' + userId + '/playlists/' + id + '/images', {
                    method: 'PUT',
                    headers: {
                        'Authorization': 'Bearer ' + token,
                ...
                // Does nothing if fails to upload image
            }
            ...
            if ('items' in data) {
                $.each(data.items, function (index, value) {
                    if (value.tracks && value.tracks.href) {
                        arr.push({
                            ...
                            images: value.images[0].url, // TODO: improve this
                            tracks: []
                        });
                    }
                });
                ...
                if (data.tracks && data.tracks.href) {
                    arr.push({
                        ...
                        images: data.images[0].url, // TODO: improve this
                        tracks: []
                    });
                }
            }
